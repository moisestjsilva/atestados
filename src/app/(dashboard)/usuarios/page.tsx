...
export default function UsuariosPage() {
  ...
  async function updateUser(id: string, data: Partial<{ status: UserStatus; role: UserRole }>) {
    setSaving(id)
            ...
            <span key={u.id} style={{ marginLeft: '0.5rem' }}>
              ...
              {isSuperAdmin && (
                <span style={{ marginLeft: '0.5rem', display: 'inline-flex', gap: '0.25rem' }}>
                  <button onClick={() => updateUser(u.id, { status: 'ATIVO' })} disabled={saving === u.id} className="btn btn-sm" style={{ background: 'hsl(142 71% 45%)', color: 'white', padding: '0.15rem 0.5rem' }}>Aprovar</button>
                  <button onClick={() => updateUser(u.id, { status: 'RECUSADO' })} disabled={saving === u.id} className="btn btn-danger btn-sm" style={{ padding: '0.15rem 0.5rem' }}>Recusar</button>
                </span>
              )}
            </span>
          ))}
              ...
              <tbody>
                {users.map(user => (
                  <tr key={user.id}>
                    ...
                    {isSuperAdmin && session?.user?.id !== user.id && (
                      <td>
                        <div style={{ display: 'flex', gap: '0.375rem' }}>
                          {user.status === 'PENDENTE' && (
                            <>
                              <button onClick={() => updateUser(user.id, { status: 'ATIVO' })} disabled={saving === user.id} className="btn btn-sm" style={{ background: 'hsl(142 71% 45%)', color: 'white' }} title="Aprovar">
                                <CheckCircle size={13} />
                              </button>
                              <button onClick={() => updateUser(user.id, { status: 'RECUSADO' })} disabled={saving === user.id} className="btn btn-danger btn-sm" title="Recusar">
                                <XCircle size={13} />
                              </button>
                            </>
                          )}
                          {user.status === 'ATIVO' && (
                            <button onClick={() => updateUser(user.id, { status: 'BLOQUEADO' })} disabled={saving === user.id} className="btn btn-secondary btn-sm" title="Bloquear">
                              <Ban size={13} />
                            </button>
                          )}
                          {user.status === 'BLOQUEADO' && (
                            <button onClick={() => updateUser(user.id, { status: 'ATIVO' })} disabled={saving === user.id} className="btn btn-sm" style={{ background: 'hsl(142 71% 45%)', color: 'white' }} title="Desbloquear">
                              <CheckCircle size={13} />
                            </button>
                          )}
                        </div>
                    ...
                    {isSuperAdmin && session?.user?.id === user.id && <td><span style={{ fontSize: '0.75rem', color: 'hsl(var(--muted-foreground))' }}>Você</span></td>}
                  </tr>
                ))}
              </tbody>
            </table>
