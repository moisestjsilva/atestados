...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  await createAuditLog({ userId: user.id, userName: user.name, action: AUDIT_ACTIONS.SETOR_EDITADO, resource: 'departments', resourceId: id, details: parsed.data })
...
export async function DELETE(_req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const employeeCount = await prisma.employee.count({ where: { departmentId: id, deletedAt: null } })
  if (employeeCount > 0) {
    return apiError(`Setor possui ${employeeCount} funcionário(s) vinculado(s). Transfira-os antes de excluir.`, 400)
  }

  await prisma.department.delete({ where: { id } })
  await createAuditLog({ userId: user.id, userName: user.name, action: AUDIT_ACTIONS.SETOR_EXCLUIDO, resource: 'departments', resourceId: id, details: { name: dept.name } })
